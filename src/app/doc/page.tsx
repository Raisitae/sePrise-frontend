...
import { getDateOnly, getTimeOnly } from "@/utils/parseTime";
...
export default function Docinterface() {
  ...
  const handleChangeMedico = (dni: any) => {
    setMedico(dni);
  };
  ...
  useEffect(() => {
    const fetchPatients = async () => {
      ...
      try {
        const agenda = await getAgendaDiaMedico(parseInt(medico), now);
    ...
    <div className="flex min-h-[100dvh] flex-col bg-gray-100 dark:bg-gray-950">
      <header className="flex h-20 items-center justify-between px-6 shadow-sm dark:bg-gray-800">
        ...
        <div className="flex flex-row gap-3">
          <FormControl className="flex flex-row gap-3 p-2">
            <TextField
              label=""
              placeholder="Dni del doctor"
              value={medico}
              variant="outlined"
              onChange={(e) => handleChangeMedico(e.target.value)}
              size="small"
            />
            <Button
              endIcon={<SearchIcon />}
              ...
              size="small"
            />
            <Button variant="outlined" href="/doctor">
              Volver
            </Button>
          </FormControl>
        </div>
