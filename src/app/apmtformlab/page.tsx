...
export default function RequestAppointmentDoc() {
  ...
  const handleDayChosen = async (fecha: string, dni: number) => {
    setIsDiaSet(false);
    const result = await getAgendaHoy(fecha);

    const filteredAgendas = result.filter(
      (item: any) => item.dni_medico === null
    );

    let timeOnlyAgendaDia = filteredAgendas.map((turno: any) => {
